...
- ✅ **通用架构** - 人机交互逻辑在 AgentExecutor 层，所有 Strategy 自动支持
...
└─────────────────────────────────────────────────────────────┘
                            ...
                            ↓ 委托
┌─────────────────────────────────────────────────────────────┐
│              ReActStrategy / Other Strategies               │
│  只关注核心执行逻辑，不需要关心人机交互细节                    │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  execute()                                          │   │
│  │  ┌──────────────────────────────────────────────┐   │   │
│  │  │  // 正常的 ReAct 循环                         │   │   │
│  │  │  for (ToolCall toolCall : aiMessage.toolCalls) {│ │ │
│  │  │    middleware.beforeToolCall(toolCall, ctx);   │   │   │
│  │  │                                              │   │   │
│  │  │    // 检查是否触发审批                        │   │   │
...
│  │  │                                              │   │   │
│  │  │    // 正常执行工具                            │   │   │
│  │  │    ToolResult result = toolInvoker.invoke();  │   │   │
│  │  │  }                                           │   │   │
│  │  └──────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
...
| 方面 | 旧设计 | 新设计 |
|------|--------|--------|
| **职责分离** | ReActStrategy 处理人机交互 | AgentExecutor 统一处理 |
| **代码复用** | 每个 Strategy 重复实现 | 通用逻辑只写一次 |
| **Strategy 简化** | 包含人机交互逻辑 | 只关注核心执行逻辑 |
| **扩展性** | 新 Strategy 需复制代码 | 自动获得人机交互能力 |
...
│           v                                                    │
│  ┌─────────────────────────────────────┐                       │
│  │ 3. ReActStrategy.execute()          │                       │
│  │    └─> LLM 调用工具                 │                       │
│  └────────┬────────────────────────────┘                       │
│           │                                                    │
│           v                                                    │
│  ┌─────────────────────────────────────┐                       │
│  │ 4. HumanInTheLoopMiddleware 检测     │                       │
│  │    └─> 设置 context.pendingApproval  │                       │
│  └────────┬────────────────────────────┘                       │
│           │                                                    │
│           v                                                    │
│  ┌─────────────────────────────────────┐                       │
│  │ 5. ReActStrategy 检测到待审批        │                       │
│  │    └─> 返回"等待审批"响应            │                       │
│  └────────┬────────────────────────────┘                       │
│           │                                                    │
│           v                                                    │
│  ┌─────────────────────────────────────┐                       │
│  │ 6. AgentExecutor 检测审批需求        │                       │
│  │    └─> 保存 checkpoint              │                       │
│  │    └─> 返回等待审批响应              │                       │
│  └────────┬────────────────────────────┘                       │
│           │                                                    │
...
└──────────────────────────────────────────────┼─────────────────┘
                                                       ...
                                                       v
┌─────────────────────────────────────────────────────────────────┐
│  8. 调用 agent.resume(checkpointId, approval)                      │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ AgentExecutor.resume() (通用逻辑)                          │    │
│  │   └─> 从 checkpoint 恢复状态                               │    │
│  │   └─> 根据决策处理：                                       │    │
│  │        • ALLOW: 执行工具                                   │    │
│  │        • REJECT: 拒绝原因反馈给 LLM                        │    │
│  │        • EDIT: 使用修改后的参数执行工具                     │    │
│  │   └─> 继续执行 strategy.execute()                          │    │
│  └────────┬────────────────────────────────────────────────┘    │
│           │                                                      │
│           v                                                      │
│  ┌─────────────────────────────────────────────────────────┐    │
